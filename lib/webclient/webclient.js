...
module.exports = WebClient = function(options) {
  ...
  if (options.logger) {
    this.log = bunyan.createLogger(options.logger);
  }
  else {
    if (this._DEBUG) {
      this.log = bunyan.createLogger({
        name: 'node-voisus.webclient',
...
WebClient.prototype.connect = function connect(name) {
...
WebClient.prototype._emit = function _emit(type, data) {
  this._debug('WebClient emitting ['+type+']: ', data);
  this.emit(type, data);
};
...
WebClient.prototype._send = function _send(type, method, data) {
  ...
  this._debug('WebClient sending ['+type+']: ', msg);
  this._socket.emit(type, msg);
}

WebClient.prototype._debug = function _debug(msg, data) {
  if (this._DEBUG) {
...
WebClient.prototype._sendHeartbeat = function _sendHeartbeat() {
  this._send(SMSG.response, 'client_heartbeat');
};
...
WebClient.prototype._parseResponseHandler = function _parseResponseHandler(data) {
  ...
  if (!method) {
    this._debug("Unrecognized client response ", data);
    this._emit(EMSG.unknownMsg, data);
  }
  this._debug(method,"response:",d);
  switch (method) {
